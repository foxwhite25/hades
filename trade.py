...
!s - 显示你目前的订单
如果要得到更多资讯请打 !help <指令>'''
helpo = '''用法: !o <你有的等级> <你需要的等级> <你拥有的数量><需要的种类代码（见下）> 
...
class CardRecordDAO:
    ...
    def _create_table(self):
        with self.connect() as conn:
            ...
            conn.execute(
                ...
                "(qid INT NOT NULL,rs INT NOT NULL,time INT NOT NULL,stat INT NOT NULL, fir TEXT NOT NULL, sec TEXT, thi TEXT , fou TEXT , PRIMARY KEY(qid))"
            )

    def checkque(self, rs):
        with self.connect() as conn:
            r = conn.execute(
    ...
    def modifypos(self, qid, uid, pos):
        ...
        if pos == "sec":
            with self.connect() as conn:
                conn.execute(
                    "UPDATE que SET sec=? WHERE qid=? AND stat=0", (data, qid),
                )
        if pos == "thi":
            with self.connect() as conn:
                conn.execute(
                    "UPDATE que SET thi=? WHERE qid=? AND stat=0", (data, qid),
                )
        if pos == "fou":
            with self.connect() as conn:
                conn.execute(
    ...
    def start(self, rs):
        qid = self.checkqbyrs(rs)[0]
        with self.connect() as conn:
            conn.execute(
                "UPDATE que SET stat=1 WHERE qid=?", (qid,)
            )
